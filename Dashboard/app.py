...
def risk_table(df):
    ...
    def hl(val):
        m = {"HIGH":"background-color:#3d0000;color:#ff6b6b",
             "MEDIUM-HIGH":"background-color:#3d2600;color:#f4a261",
             "MEDIUM":"background-color:#3d3400;color:#e9c46a"}
        return m.get(val,"")

    st.dataframe(disp.style.applymap(hl,subset=["Risk Level"]),
...
def model_chart(metrics):
    ...
    for i, name in enumerate(names):
        fig.add_trace(go.Bar(
            ...
            textposition="outside", textfont=dict(size=10,color="#e8f4fd")
        ))
    fig.update_layout(
        ...
        title=dict(text=f"Best model: {metrics.get('best_model','')}",
...
def trend_chart():
    ...
    fig.update_layout(
        height=300, paper_bgcolor="#0a0e1a", plot_bgcolor="#0a0e1a",
        font=dict(color="#e8f4fd",family="IBM Plex Sans"),
        legend=dict(bgcolor="#0d1b2a",bordercolor="#1e3a5f"),
        xaxis=dict(gridcolor="#1e3a5f"),
        ...
        yaxis2=dict(title="Cost ($B)",overlaying="y",side="right",
                    gridcolor="#1e3a5f"),
        annotations=[dict(
            x=2024, y=51, text="$121B in 2024\n(ORNL)", showarrow=True,
            arrowhead=2, font=dict(color="#e63946",size=10)
        )]
    )
    st.plotly_chart(fig, use_container_width=True)
...
def risk_calculator():
    ...
    st.caption("Estimate risk for a custom scenario")
    ...
    if st.button("⚡ Calculate Risk", type="primary"):
        risk = min(1.0, (
            (wind/100)*0.25 + (snow/48)*0.15 + (0.20 if ice_storm else 0) +
        ...
        <div style='background:#0d1b2a;border:2px solid {col};border-radius:12px;
            ...
            <div style='color:#e8f4fd;font-size:0.85rem;margin-top:8px;'>
                {state} | {season} | Wind {wind}mph | Infra Age {age}yrs
            </div>
...
def main():
    ...
    fdf = df.copy()
    if state       != "All": fdf = fdf[fdf["state"]      == state]
    if risk_filter != "All": fdf = fdf[fdf["risk_level"] == risk_filter]
    ...
    with col_l: risk_map(fdf)
    with col_r: risk_table(fdf)

    st.divider()
    col_a, col_b = st.columns(2)
    with col_a: trend_chart()
    with col_b: model_chart(metrics)
